...
### ⚠️ **バッテリー機能実装 - 複数回の試行錯誤記録** (2025-11-07)
...
#### **試行2: イベントリスナー条件付き登録**
- **アプローチ**: `#if IS_ENABLED(CONFIG_PROSPECTOR_BATTERY_SUPPORT)`でイベント登録を囲む
- **結果**: ❌ 改善なし

#### **試行3: Widget初期化安全チェック**
- **アプローチ**: `if (!scanner_battery_widget.obj) return;`
- **結果**: ❌ 改善なし

#### **試行4: バッテリーハードウェア実在性チェック** (commit 1b3a585)
- **アプローチ**: `device_is_ready()`でハードウェア存在確認
- **実装**: USB-onlyモードへのグレースフルフォールバック
- **結果**: ❌ ビルドエラー（unterminated #if）

#### **試行5: プリプロセッサディレクティブ修正** (commit 7a10c7e)
- **問題**: Line 134の`#if`に対応する`#endif`が欠落
- **修正**: Line 302に`#endif // CONFIG_PROSPECTOR_BATTERY_SUPPORT`追加
- **追加修正**: 複雑なハードウェアセンサー読み取りコード削除（95行削減）
- **結果**: ⚠️ ビルド成功見込み、動作未確認

#### **累積的な問題点**

1. **Device Tree vs 実ハードウェア不一致**
   - Seeeduino XIAO BLEボードにはDT定義でバッテリーノードあり
   - 実際のProspectorスキャナーにはバッテリーハードウェアなし
   - `DT_HAS_CHOSEN(zmk_battery)` = TRUE でも物理デバイスなし

2. **ZMKバッテリーシステムの複雑性**
   - ZMK内部キャッシュシステム（60秒更新）
   - センサーチャンネル互換性問題
   - イベント駆動 vs ポーリング更新の違い

3. **コード複雑度の増大**
   - ハードウェア直接読み取り実装で100行以上追加
   - デバッグコードが本番コードに混在
   - メンテナンス困難な状態

#### **技術的学習事項**

**Critical Lesson**: 段階的実装の重要性
- 最初から複雑な機能を追加せず、シンプルなZMK API使用に留めるべきだった
- デバッグコードは別ブランチで管理すべき
- ハードウェア実機テストなしでの複雑実装は危険

**実装すべきだった順序**:
1. ✅ Kconfigデフォルト値修正（default n）
2. ✅ バッテリーハードウェア存在チェック
3. ❌ シンプルなZMK API使用のみ（zmk_battery_state_of_charge()）
4. ❌ 動作確認後に段階的機能追加
...
- 実ハードウェアでのテストが必要
- コード複雑度を下げる必要あり
- より基本的な機能（チャンネル機能等）を優先

**将来の再実装時の方針**:
1. シンプルなZMK API使用のみから開始
2. 実ハードウェアで動作確認
3. 段階的に機能追加
4. デバッグコードを本番コードから分離

**記録コミット**:
- Module: `1b3a585` (ハードウェアチェック), `7a10c7e` (#if修正)
- Config: `2723475`, `cdffc45` (ビルドトリガー)

---

**Development Start**: 2025-11-07
**Status**: ⚠️ **BATTERY FEATURE SUSPENDED** - チャンネル機能は完成、バッテリー機能は保留
**Next Step**: 別の優先事項に移行（ユーザー判断待ち）