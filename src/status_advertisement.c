...
static void start_custom_advertising(void) {
    ...
    if (!adv_set) {
        int err = bt_le_ext_adv_create(BT_LE_ADV_NCONN_IDENTITY, NULL, &adv_set);
        if (err) {
            LOG_ERR("❌ Failed to create extended advertising set: %d", err);
            return;
        }
        LOG_INF("✅ Extended advertising set created");
    }
...
static void adv_work_handler(struct k_work *work) {
    ...
    // Update existing advertising data
    int err = bt_le_ext_adv_set_data(adv_set, adv_data_array, ARRAY_SIZE(adv_data_array),
                                     scan_rsp, ARRAY_SIZE(scan_rsp));

    if (err == 0) {
        LOG_INF("✅ Extended advertising data updated successfully");
    } else {
        LOG_ERR("❌ Extended advertising update failed: %d", err);
