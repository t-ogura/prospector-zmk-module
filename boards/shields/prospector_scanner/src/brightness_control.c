...
// Simple GPIO backlight control for testing
static const struct gpio_dt_spec backlight_gpio = GPIO_DT_SPEC_GET_OR(DT_NODELABEL(xiao_d), gpios, {0});

static int set_backlight(uint8_t brightness_percent) {
    // Use D4 pin for simple GPIO backlight control
    ...
    // Configure D4 (P1.11) as output
    int ret = gpio_pin_configure(gpio_dev, 11, GPIO_OUTPUT_ACTIVE);
    if (ret < 0) {
    ...
    ret = gpio_pin_set(gpio_dev, 11, enable ? 1 : 0);
    if (ret < 0) {
        LOG_ERR("Failed to set backlight GPIO: %d", ret);
        return ret;
    }
    
    LOG_INF("Backlight %s via GPIO D4 (P1.11)", enable ? "ON" : "OFF");
    return 0;
}
